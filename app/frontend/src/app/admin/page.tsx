 ...
 * All heavy components are dynamically imported to avoid SSR issues with WebGL/canvas.
 */
...
function MapPlaceholder() {
  return (
    ...
    >
      <span className="animate-spin" style={{ marginRight: 8, fontSize: "1.2rem" }}>⬡</span>
      Loading map…
    </div>
  );
}
...
export default function AdminPage() {
  ...
  const { filters, layers, setWards, setSources, setPersistence, toggleLayer, setHorizon, resetFilters } = useFilters();
  const { hotspots, isLoading: hotspotsLoading } = useHotspots(filters);
  const { cells: fusionCells, isLoading: fusionLoading } = useFusion(hourOffset);

  // Refresh all SWR keys after an agent run completes
  ...
  const { agents, running, error: agentError, runAgent, resetAgents } = useAgentRun(onAgentComplete);
